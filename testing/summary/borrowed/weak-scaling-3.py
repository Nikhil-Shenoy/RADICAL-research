...
def read_data(name, cycle):
    ...
    md_data = []
    for i in range(len(md_times_new)):
        d_out = (md_times_done[i]  - md_times_stageout[i]).total_seconds()
        d_in  = (md_times_alloc[i] - md_times_stagein[i]).total_seconds()
        md_data.append(d_in + d_out)

    for i in range(len(exchange_times_new)):
        d_out = (exchange_times_done[i]  - exchange_times_stageout[i]).total_seconds()
        d_in  = (exchange_times_alloc[i] - exchange_times_stagein[i]).total_seconds()
        ex_data = d_in + d_out
...
def gen_graph():
    ...
    # 4 cycles
    name1 = 'execution_profile_rp.session.antons-pc.antons.016667.0023'
    ...
    # 4 cycles
    name1 = 'execution_profile_rp.session.antons-pc.antons.016667.0025'
    ...
    # 4 cycles
    name1 = 'execution_profile_rp.session.antons-pc.antons.016668.0001'
    ...
    # 4 cycles
    name1 = 'execution_profile_rp.session.antons-pc.antons.016668.0003'
    ...
    # 4 cycles
    name1 = 'execution_profile_rp.session.antons-pc.antons.016668.0004'
    ...
    # 4 cycles
    name1 = 'execution_profile_rp.session.antons-pc.antons.016668.0005'
    ...
    enmd_core_times.append(4.078770+17.237736)
    ...
    p0 = axarr[2].bar(ind-1.5*width, rp_overhead_times, width, yerr=rp_overhead_times_err, color='red', edgecolor = "white")
    p1 = axarr[2].bar(ind-0.5*width, enmd_overhead_times, width, yerr=enmd_overhead_times_err, color='darkslategray', edgecolor = "white")
    ...
    text ='Performance Characterization of T-REMD Alanine Dipeptide with Amber Kernel on SuperMIC.'
    axarr[0].set_title('\n'.join(wrap(text,70)))
    ...
    axarr[0].set_yticks(np.arange(0,2500,250))
    ...
    plt.savefig('../plots/plot-weak-scaling-3-14.08.2015.png')
