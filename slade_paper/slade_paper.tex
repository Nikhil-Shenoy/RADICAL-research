...
Traditional distributed systems utilize batch queueing systems in order to schedule jobs to the system's resources. In such systems, scripts are written to execute several different tasks sequentially, with each task being told what data and resources are available to it. Each of the tasks executes accordingly, and the script collects the output and returns the results to the user. The main advantage in using such batch systems is that, using the correct task scheduling policy, all the jobs can be executed sequentially and automatically. Each job is placed in a queue, and then the scheduling policy decides which job to run and where it should run. Many policies exist for selecting a job; among the simplest are the first-come first-serve (FCFS), which schedules jobs as the come in,  and the multi-level feedback queue (MLFQ), which shuffles jobs between several queues by the amount of remaining time left. Once a job is chosen to be run, the scheduler assigns the requested resources, if available, and the job executes, completes, and returns its results. However, this scheduling scheme can result in unused resources for the duration of the job's execution. To alleviate this problem, the concept of Pilot-Jobs was introduced. 
