...
if __name__ == "__main__":
    ...
    if len(args) < 7:
        ...
        print "python k-means k dim #tasks #cores <input file name> <report file name> <queue> <resource>"
        sys.exit(-1)
    ...
    maxIt = 2 # the maximum number of iteration
    ...
    try:
        ...
        # change their Dan Connollystate.
        ...
        for i in range(1,CUs+1):
            ...
            staged_combiner_files = {
                'source': INTERMEDIATE_FILE, 'target': 'staging:///%s' % INTERMEDIATE_FILE, 'action': rp.MOVE }
            ...
            staged_combiner_files_link = {
             'source': 'staging:///%s' % INTERMEDIATE_FILE, 'target': INTERMEDIATE_FILE, 'action': rp.COPY }
        ...
        centroids_in = {
             'source': 'staging:///%s' % INTERMEDIATE_FILE, 'target': INTERMEDIATE_FILE, 'action': rp.MOVE }
        ...
        while m<maxIt:
            ...
            for i in range(1,CUs+1):
                ...
                if Resource == "xsede.stampede":
                    cudesc.executable = "/opt/apps/intel15/python/2.7.9/bin/python"
                elif Resource == "xsede.stampede_yarn":
                    cudesc.pre_exec    = ["module purge","module load TACC","module load intel/15.0.2", "module load python/2.7.9"] # yarn only
                    cudesc.executable = "python" # yarn only
            ...
            if m==0:
                ...
                cudesc.input_staging.append(centroids_in)
        ...
        ProfFile = open('{1}-{0}.csv'.format(tasks,report_name),'w')
        ProfFile.write('CU,Name,New,StageIn,Allocate,Exec,StageOut,Done,Read,Exec,Write\n')
        for cu in cu_set:
            timing_str=[cu.uid,cu.name,'N/A','N/A','N/A','N/A','N/A','N/A']
            for states in cu.state_history:
                if states.as_dict()['state']=='New':
            ...
            ProfFile.write(timing_str[0]+','+timing_str[1]+','+
                           ...
                           timing_str[6]+','+timing_str[7]+','+cu.stdout)
